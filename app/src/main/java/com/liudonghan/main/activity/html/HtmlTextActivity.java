...
//                        }
        ...
        ADHtmlUtils.getInstance()
                .from(this)
                .url("https://home.meishichina.com/recipe-658543.html")
                .cssQuery("body", "div.wrap", "div.w.clear", "div.space_left", "div.space_box_home")
                .attrs("src")
                .listener(new ADHtmlUtils.Builder.OnConnectListener() {
                    @Override
                    public void onReady() {
                    ...
                    @Override
                    public void onDocument(Document document, Elements selectElements, String json) {
                        FoodChinaHtmlModel chinaHtmlModel = new FoodChinaHtmlModel();
                        List<FoodChinaHtmlModel.Dosing> dosings = new ArrayList<>();
                        Elements elements = selectElements.select("fieldset.particulars");
                        for (int i = 0; i < elements.size(); i++) {
                            FoodChinaHtmlModel.Dosing dosing = new FoodChinaHtmlModel.Dosing();
                            List<FoodChinaHtmlModel.Dosing.FoodChild> foodChildList = new ArrayList<>();
                            dosing.setDosingType(elements.get(i).select("legend").text());
                            Elements cssByElements = ADHtmlUtils.getInstance().findCssByElements(elements.get(i).html(), "div.recipeCategory_sub_R.clear", "ul", "li");
                            for (int j = 0; j < cssByElements.size(); j++) {
                                FoodChinaHtmlModel.Dosing.FoodChild foodChild = new FoodChinaHtmlModel.Dosing.FoodChild();
                                foodChild.setName(cssByElements.get(j).select("span.category_s1").text());
                                foodChild.setDosage(cssByElements.get(j).select("span.category_s2").text());
                                foodChildList.add(foodChild);
                            }
                            dosing.setFoodChildren(foodChildList);
                            dosings.add(dosing);
                        }
                        Elements cssByElements = ADHtmlUtils.getInstance().findCssByElements(selectElements.html(), "div.recipeCategory_sub_R.mt30.clear", "ul", "li");
                        if (!ADArrayUtils.isEmpty(cssByElements)) {
                            FoodChinaHtmlModel.Dosing dosing = new FoodChinaHtmlModel.Dosing();
                            dosing.setDosingType("其他");
                            List<FoodChinaHtmlModel.Dosing.FoodChild> foodChildList = new ArrayList<>();
                            for (int i = 0; i < cssByElements.size(); i++) {
                                FoodChinaHtmlModel.Dosing.FoodChild foodChild = new FoodChinaHtmlModel.Dosing.FoodChild();
                                foodChild.setName(cssByElements.get(i).select("span.category_s1").text());
                                foodChild.setDosage(cssByElements.get(i).select("span.category_s2").text());
                                foodChildList.add(foodChild);
                            }
                            dosing.setFoodChildren(foodChildList);
                            dosings.add(dosing);
                        }
                        Elements cssByElementsContent = ADHtmlUtils.getInstance().findCssByElements(selectElements.html(), "div.recipeStep", "ul", "li");
                        List<FoodChinaHtmlModel.Step> steps = new ArrayList<>();
                        for (int i = 0; i < cssByElementsContent.size(); i++) {
                            FoodChinaHtmlModel.Step step = new FoodChinaHtmlModel.Step();
                            step.setImg(ADHtmlUtils.getInstance().findCssByElements(cssByElementsContent.get(i).html(), "div.recipeStep_img", "img").attr("src"));
                            step.setContent(cssByElementsContent.get(i).select("div.recipeStep_word").text());
                            steps.add(step);
                        }
                        chinaHtmlModel.setDosing(dosings);
                        chinaHtmlModel.setStep(steps);
                        Log.i("美食天下", "配料数据：" + ADGsonUtils.toJson(chinaHtmlModel));
                    }
